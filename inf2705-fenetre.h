#ifndef __INF2705_FENETRE_H__
#define __INF2705_FENETRE_H__

///////////////////////////////////////////////////////////////////////////
/// @class FenetreTP
/// @brief Classe pour créer une fenêtre OpenGL
///
///        Fonctions utilitaires pour créer une fenêtre OpenGL
///
...
public:
...
#if defined( __APPLE__ )
        // Demander le profile OpenGL Core 4.1 pour Apple
        SDL_GL_SetAttribute( SDL_GL_CONTEXT_PROFILE_MASK, SDL_GL_CONTEXT_PROFILE_CORE );
        SDL_GL_SetAttribute( SDL_GL_CONTEXT_MAJOR_VERSION, 4 );
        SDL_GL_SetAttribute( SDL_GL_CONTEXT_MINOR_VERSION, 1 );
#else
        // Demander le profile OpenGL Compatibility 4.x pour les autres
    ...
    static std::string composerNomFichier( std::string type )
    {
        auto now = std::chrono::system_clock::now();
        auto in_time_t = std::chrono::system_clock::to_time_t(now);
        std::stringstream ss;
        ss << std::put_time( std::localtime(&in_time_t), "sauvegardeFenetre_%Y%m%d_%H%M%S." ) << type;
        return ss.str();
    }

    // sauvegarder une copie de la fenêtre OpenGL
    void sauvegarderFenetreEnTGA( )
    {
        std::cout << "Sauvegarder en TGA ... ";
        unsigned char *pixels = new unsigned char[3*largeur_*hauteur_];
        if ( pixels )
        {
            // préciser l'alignement des pixels en mémoire
            glPixelStorei( GL_PACK_ALIGNMENT, 1 );
            glPixelStorei( GL_UNPACK_ALIGNMENT, 1 );
            // obtenir les pixels
            GLint readBuffer; glGetIntegerv( GL_READ_BUFFER, &readBuffer );
            glReadBuffer( GL_FRONT );
            glReadPixels( 0, 0, largeur_, hauteur_, GL_RGB, GL_UNSIGNED_BYTE, pixels );
            glReadBuffer( readBuffer );

            // écrire le fichier
            char fichier[100];
            strncpy( fichier, composerNomFichier( "tga" ).c_str(), sizeof(fichier) );
            FILE *fp = fopen( fichier, "w" );
            if ( fp != NULL )
            {
                short  TGAhead[] = { 0, 2, 0, 0, 0, 0, (short)largeur_, (short)hauteur_, 24 };
                fwrite( &TGAhead, sizeof(TGAhead), 1, fp );
                if ( fwrite( pixels, 3*largeur_*hauteur_, 1, fp ) == 1 )
                    std::cout << "Fichier sauvegardé correctement : " << fichier << std::endl;
                else
                    std::cout << "ERREUR à l'écriture du fichier : " << fichier << std::endl;
                fclose( fp );
            }
    ...
    {
        // sauvegarderFenetreEnPNG( );
        // sauvegarderFenetreEnTGA( );
    }

    // Afficher à l'écran le contenu du fichier donné
    static void imprimerFichier( std::string fn )
    {
        // Ouvrir le fichier
        std::ifstream fichier( fn );
        ...
        // Ecrire le contenu du fichier, précédé de l'entête
        std::cout << contenuFichier.str() << std::endl;
    }
